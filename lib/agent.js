...
const { WEBAUTHN_SK_ECDSA } = require('./protocol/constants.js');
      ...
      while (p < bufferLen) {
        ...
        if (this[SYM_MODE] === ROLE_CLIENT) {
          ...
          switch (msgType) {
            ...
            default: {
              // Skip past the message body — without this, unknown message
              // types (e.g. SSH_AGENTC_EXTENSION=27) leave their body bytes
              // in the buffer, corrupting subsequent message parsing.
              p += this[SYM_MSGLEN] - 1;
              const req = new AgentInboundRequest(msgType);
              this[SYM_REQS].push(req);
    ...
    signReply(req, signature) {
      ...
      */
      ...
      /*
        ...
        For webauthn-sk-ecdsa, the wire format (per PROTOCOL.u2f) is different:

          string    "webauthn-sk-ecdsa-sha2-nistp256@openssh.com"
          string    ecdsa_signature   (R + S as mpints)
          byte      flags
          uint32    counter
          string    origin
        ...
        The ecdsa_signature is wrapped in a string (sshbuf_froms on the server),
        but the remaining fields (flags, counter, etc.) are FLAT in the outer
        buffer — NOT inside a string wrapper. If the entire blob is wrapped in
        one string, the server's sshbuf_froms consumes everything as the ECDSA
        sub-buffer, leaving nothing for the flat fields.

        This matches the equivalent fix in Protocol.js for direct connections
        (see rado0x54/ssh2#1).
      */
      ...
      const isWebauthnSK = sigFormat === WEBAUTHN_SK_ECDSA;
      const innerLen = sigBlockLen(isWebauthnSK, sigFormatLen, signature.length);

      const buf = Buffer.allocUnsafe(4 + 1 + innerLen);

      writeUInt32BE(buf, buf.length - 4, p);
      buf[p += 4] = SSH_AGENT_SIGN_RESPONSE;

      writeSigBlock(buf, ++p, isWebauthnSK, sigFormat, sigFormatLen, signature);
