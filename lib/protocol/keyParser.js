...
const { eddsaSupported, SUPPORTED_CIPHER, WEBAUTHN_SK_ECDSA } = require('./constants.js');
const {
  bufferSlice,
  makeBufferParser,
  readString,
...
class WebAuthnSKECDSAKey {
  ...
  getPublicSSH() {
    const algo = Buffer.from(WEBAUTHN_SK_ECDSA);
  ...
  sign() {
    return new Error('WebAuthn SK keys must be signed via agent');
  }

  verify() {
    return new Error('Verification not implemented for WebAuthn SK keys');
  }
  ...
  equals(key) {
    ...
    return this._ecPoint.equals(key._ecPoint)
           && this._application === key._application;
  }
}
...
function parseKey(data, passphrase) {
  ...
  if (origBuffer) {
    ...
    // Accept both the canonical webauthn-sk-ecdsa algorithm name and the
    // shorter sk-ecdsa variant (when the application field identifies it
    // as a WebAuthn key). OpenSSH 10.3 canonicalizes the algorithm name
    // in SSH_AGENTC_SIGN_REQUEST key blobs from webauthn-sk-ecdsa to
    // sk-ecdsa (both map to KEY_ECDSA_SK internally).
    //
    // The key formats are identical, but the SIGNATURE formats differ:
    //   sk-ecdsa (app="ssh:"):        ecdsa_sig + flags + counter
    //   webauthn-sk-ecdsa (app=web):  ecdsa_sig + flags + counter +
    //                                 origin + clientDataJSON + extensions
    //
    // We distinguish them by the application field: standard FIDO2 keys
    // always use "ssh:", while WebAuthn keys use a web domain (e.g.,
    // "localhost"). Only treat sk-ecdsa as webauthn when app != "ssh:".
    if (type === WEBAUTHN_SK_ECDSA
        || type === 'sk-ecdsa-sha2-nistp256@openssh.com') {
      const curve = binaryKeyParser.readString(true);
      const ecPoint = binaryKeyParser.readString();
      const application = binaryKeyParser.readString(true);
      binaryKeyParser.clear();

      // Standard FIDO2 sk-ecdsa keys (app="ssh:") are NOT webauthn keys
      // and must not be parsed as WebAuthnSKECDSAKey — the signature
      // formats are incompatible. Fall through to the generic path which
      // will return "Unsupported key format" (ssh2 does not support
      // standard sk-ecdsa).
      // Explicitly webauthn if the algorithm says so, OR if the algorithm
      // was canonicalized to sk-ecdsa but the application is a web domain
      // (not the standard FIDO2 "ssh:" relying party).
      const isWebauthn = type === WEBAUTHN_SK_ECDSA
        || (type === 'sk-ecdsa-sha2-nistp256@openssh.com'
            && application !== 'ssh:');
      if (isWebauthn
          && curve === 'nistp256'
          && ecPoint && ecPoint.length === 65 && ecPoint[0] === 0x04) {
        return new WebAuthnSKECDSAKey(ecPoint, application);
      }
      if (isWebauthn) {
        return new Error('Malformed WebAuthn SK ECDSA public key');
      }
      // Not webauthn — fall through to generic parsing
    }
    if (type !== undefined) {
