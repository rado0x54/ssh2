...
module.exports = {
  ...
  /**
   * Calculate the byte length of an SSH signature block for the given algorithm.
   *
   * For standard types (rsa, ecdsa, ed25519):
   *   string algo + string sig_data
   *
   * For webauthn-sk-ecdsa (per PROTOCOL.u2f):
   *   string algo + <raw sig_data>  (no inner string wrapper)
   *
   * The webauthn signature blob from the agent contains a compound structure
   * (string ecdsaSig + byte flags + uint32 counter + string origin + ...).
   * The ECDSA sub-signature is wrapped in its own string (for sshbuf_froms on
   * the server), but the remaining fields are flat in the outer buffer. If the
   * entire blob were wrapped in another string, the server's first sshbuf_froms
   * would consume everything as the ECDSA sub-buffer, leaving nothing for
   * flags/counter/origin/clientData/extensions.
   *
   * @param {boolean} isWebauthnSK - true if the algorithm is webauthn-sk-ecdsa
   * @param {number} algoLen - byte length of the algorithm name string
   * @param {number} sigLen - byte length of the signature data
   * @returns {number} total byte length of the signature block (including the
   *   outer uint32 length prefix)
   */
  sigBlockLen: (isWebauthnSK, algoLen, sigLen) => {
    // outer uint32 + string(algo) + [string(sig) or raw sig]
    return isWebauthnSK
      ? (4 + 4 + algoLen + sigLen)
      : (4 + 4 + algoLen + 4 + sigLen);
  },
  /**
   * Write an SSH signature block into a buffer at the given offset.
   *
   * @param {Buffer} buf - target buffer (must be pre-allocated with enough space)
   * @param {number} p - write offset (pointing to where the outer uint32 goes)
   * @param {boolean} isWebauthnSK - true if the algorithm is webauthn-sk-ecdsa
   * @param {string} algo - algorithm name string
   * @param {number} algoLen - byte length of the algorithm name
   * @param {Buffer} signature - signature data
   * @returns {number} new offset after the written block
   */
  writeSigBlock: (buf, p, isWebauthnSK, algo, algoLen, signature) => {
    const sigLen = signature.length;
    if (isWebauthnSK) {
      // WebAuthn: string(algo) + raw sig bytes (no inner string wrapper)
      writeUInt32BE(buf, 4 + algoLen + sigLen, p);
      writeUInt32BE(buf, algoLen, p += 4);
      buf.utf8Write(algo, p += 4, algoLen);
      buf.set(signature, p += algoLen);
      return p + sigLen;
    }
    // Standard: string(algo) + string(sig)
    writeUInt32BE(buf, 4 + algoLen + 4 + sigLen, p);
    writeUInt32BE(buf, algoLen, p += 4);
    buf.utf8Write(algo, p += 4, algoLen);
    writeUInt32BE(buf, sigLen, p += algoLen);
    buf.set(signature, p += 4);
    return p + sigLen;
  },
