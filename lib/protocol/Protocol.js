...
class Protocol {
  ...
  authPK(username, pubKey, keyAlgo, cbSign) {
    ...
    cbSign(packet, (signature) => {
      ...
      // DEBUG: Dump the full signature block on the wire
      // Wire format: uint32 total_sig_len + string algo + string sig_data
      console.log('[Protocol.js DEBUG] === WIRE SIGNATURE BLOCK ===');
      console.log('[Protocol.js DEBUG] algo in packet:', keyAlgo);
      console.log('[Protocol.js DEBUG] sig_data (' + sigLen + ' bytes):', Buffer.from(signature).toString('hex'));
