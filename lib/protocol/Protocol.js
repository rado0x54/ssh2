...
class Protocol {
  ...
  authPK(username, pubKey, keyAlgo, cbSign) {
    ...
    cbSign(packet, (signature) => {
      ...
      const isWebauthnSK = keyAlgo === WEBAUTHN_SK_ECDSA;
      // For webauthn-sk: sig block = uint32 + string algo + raw sig
      // For standard:    sig block = uint32 + string algo + string sig
      const sigBlockLen = isWebauthnSK
        ? (4 + 4 + algoLen + sigLen)
      ...
      if (isWebauthnSK) {
        // WebAuthn SK signature wire format (OpenSSH PROTOCOL.u2f):
        //   string algo + raw sig (no inner string wrapper)
        ...
        // Standard SSH signature format: string algo + string sig_data
