...
class Protocol {
  ...
  authPK(username, pubKey, keyAlgo, cbSign) {
    ...
    console.log('[Protocol.js DEBUG] data-to-sign (' + packet.length + ' bytes):', Buffer.from(packet).toString('hex').slice(0, 400));
    console.log('[Protocol.js DEBUG] pubKey blob (' + pubKeyLen + ' bytes):', Buffer.from(pubKey).toString('hex'));
    cbSign(packet, (signature) => {
      console.log('[Protocol.js DEBUG] cbSign called, keyType:', keyType, 'keyAlgo:', keyAlgo);
      console.log('[Protocol.js DEBUG] signature from agent (' + signature.length + ' bytes):', Buffer.from(signature).toString('hex').slice(0, 300));
      signature = convertSignature(signature, keyType);
      console.log('[Protocol.js DEBUG] signature after convertSignature (' + (signature ? signature.length : 'false') + ' bytes):', signature ? Buffer.from(signature).toString('hex').slice(0, 300) : 'false');
      if (signature === false)
        throw new Error('Error while converting handshake signature');

      const sigLen = signature.length;
      const isWebauthnSKForAlloc = keyAlgo === 'webauthn-sk-ecdsa-sha2-nistp256@openssh.com';
      // For webauthn-sk: sig block = uint32 + string algo + raw sig (no inner string wrapper)
      // For standard:    sig block = uint32 + string algo + string sig
      const sigBlockLen = isWebauthnSKForAlloc
        ? (4 + 4 + algoLen + sigLen)
        : (4 + 4 + algoLen + 4 + sigLen);
      p = this._packetRW.write.allocStart;
      packet = this._packetRW.write.alloc(
        1 + 4 + userLen + 4 + 14 + 4 + 9 + 1 + 4 + algoLen + 4 + pubKeyLen + sigBlockLen
      );
      ...
      // For webauthn-sk keys, the signature wire format is:
      //   string algo + string(ecdsaSig) + byte flags + uint32 counter + string origin + string clientData + string extensions
      // NOT string algo + string(entire_blob). The ecdsaSig is a nested string,
      // but the remaining fields are flat in the outer buffer.
      // Go's ssh.Signature serializes as: string Format + string Blob + <raw Rest>
      //
      // The agent returns: string(R+S) + flags + counter + origin + clientData + extensions
      // We write: uint32 totalLen + string algo + <agent's return verbatim>
      // This produces: string algo + string(R+S) + flags + counter + ... in the outer buffer
      const isWebauthnSK = keyAlgo === 'webauthn-sk-ecdsa-sha2-nistp256@openssh.com';

      if (isWebauthnSK) {
        // Total sig block = string algo + raw signature (NOT string-wrapped)
        writeUInt32BE(packet, 4 + algoLen + sigLen, p += pubKeyLen);
        writeUInt32BE(packet, algoLen, p += 4);
        packet.utf8Write(keyAlgo, p += 4, algoLen);
        packet.set(signature, p += algoLen);
        p += sigLen;
      } else {
        // Standard format: string algo + string sig_data
      ...
      console.log('[Protocol.js DEBUG] === WIRE SIGNATURE BLOCK ===');
      console.log('[Protocol.js DEBUG] algo in packet:', keyAlgo);
      console.log('[Protocol.js DEBUG] isWebauthnSK:', isWebauthnSK);
      console.log('[Protocol.js DEBUG] sig_data (' + sigLen + ' bytes)');
      console.log('[Protocol.js DEBUG] === END WIRE SIGNATURE BLOCK ===');
