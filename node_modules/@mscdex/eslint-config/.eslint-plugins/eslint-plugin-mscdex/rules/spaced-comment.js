...
function escapeRegExp(s) {
  ...
  // when the simpler form would be disallowed by Unicode patterns’ stricter
  // grammar.
	return s
		.replace(/[|\\{}()[\]^$+*?.]/g, '\\$&')
		.replace(/-/g, '\\x2d');
 ...
 */
function createExceptionsPattern(exceptions, isPrefix) {
    let pattern = "";
 ...
 * @param {string[]} markers A marker list.
 * @param {string[]} exceptions An exception pattern list.
 * @param {string[]} prefixExceptions A prefix exception pattern list.
 * @returns {RegExp} A RegExp object for the beginning of a comment in `always` mode.
 */
function createAlwaysStylePattern(markers, exceptions, prefixExceptions) {
    let pattern = "^";
    ...
    pattern += "?"; // or nothing.
    const exceptPatternsPre = createExceptionsPattern(prefixExceptions, true);
    const exceptPatterns = createExceptionsPattern(exceptions, false);
    if (prefixExceptions.length > 0) {
      if (exceptions.length === 0)
        pattern += exceptPatternsPre;
      else
        pattern += `(?:(?:${exceptPatternsPre})|(?:${exceptPatterns}))`;
    } else {
      pattern += exceptPatterns;
    }
...
module.exports = {
    ...
    create(context) {
        ...
        const styleRules = ["block", "line"].reduce((rule, type) => {
            ...
            const prefixExceptions = config[type] && config[type].prefixExceptions || config.prefixExceptions || [];
            const endNeverPattern = "[ \t]+$";

            // Create RegExp object for valid patterns.
            rule[type] = {
                beginRegex: requireSpace ? createAlwaysStylePattern(markers, exceptions, prefixExceptions) : createNeverStylePattern(markers),
                endRegex: balanced && requireSpace ? new RegExp(`${createExceptionsPattern(exceptions, false)}$`, "u") : new RegExp(endNeverPattern, "u"),
                hasExceptions: exceptions.length > 0 || prefixExceptions.length > 0,
